package utils.ir.dataset.format;
...
/**
 * Modified from Mark J. Hoy (mhoy@cs.cmu.edu)'s WarcRecord. I fixed bugs in outputting warc record headers. Use readNextWarcRecordMod() and writeMod() for
 * reading and writing warc record.
 * 
 * @author mhoy
 * @author Jiepu Jiang (last modified)
 * @version Jun 24, 2012
 */
public class WarcRecord {
	...
	public static WarcRecord readNextWarcRecordMod( DataInputStream in ) throws IOException {
		// LOG.info("Starting read of WARC record");
		StringBuffer recordHeader = new StringBuffer();
		byte[] recordContent = readNextRecord( in, recordHeader );
		if ( recordContent == null ) {
		...
		// set the content
		retRecord.setContent( recordContent );
		retRecord.warcHeaderText = recordHeader.toString();
	...
	private String warcHeaderText = null; // add by Jiepu (in order to output the original warc header read from the
											// dataset file)
	...
	/**
	 * Incorrect implementation. The outputted warc record cannot even be parsed by this class itself.
	 * 
	 * @param out
	 * @throws IOException
	 */
	...
	/**
	 * This is the correct way to output a warc record.
	 * 
	 * @param out
	 * @throws IOException
	 */
	public void writeMod( DataOutput out ) throws IOException {
		out.write( WARC_VERSION_LINE.getBytes() );
		out.write( warcHeaderText.getBytes() );
		out.write( "\n".getBytes() );
		out.write( warcContent );
		if ( this.getHeaderRecordType().equals( "warcinfo" ) ) {
			out.write( "\n".getBytes() );
		}
	}
